import { http, HttpOptions } from "@next-core/brick-http";
import { ResponseBodyWrapper } from "../../../wrapper";

export interface PermissionApi_ValidateEasyhubServerPermissionRequestParams {
  /** 需要鉴权的用户名 */
  user: string;

  /** 需要鉴权的权限点 */
  action: string;

  /** 如果一个权限点有数据权限,但是不想校验数据权限,这个参数传true，默认为false */
  validate_action_only?: boolean;
}

export interface PermissionApi_ValidateEasyhubServerPermissionResponseBody {
  /** 是否鉴权成功 */
  accepted?: boolean;
}

/**
 * @description 校验easyhub_server权限
 * @endpoint GET /api/v1/permission/validate
 */
export const PermissionApi_validateEasyhubServerPermission = async (
  params: PermissionApi_ValidateEasyhubServerPermissionRequestParams,
  options?: HttpOptions
): Promise<PermissionApi_ValidateEasyhubServerPermissionResponseBody> =>
  /**! @contract easyops.api.permission.permission.ValidateEasyhubServerPermission */ (
    await http.get<
      ResponseBodyWrapper<PermissionApi_ValidateEasyhubServerPermissionResponseBody>
    >(
      "api/gateway/permission.permission.ValidateEasyhubServerPermission/api/v1/permission/validate",
      { ...options, params }
    )
  ).data;
